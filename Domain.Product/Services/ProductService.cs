...
namespace Domain.Product.Services
{
    public class ProductService : IProductService
    {
        ...
        public async Task<ResponseDTO<IEnumerable<ProductDTO>>> GetAllProducts(string brand)
        {
            var products = await _productRepository.GetProductAsync(brand);
            var productDto = new List<ProductDTO>();
            productDto = _mapper.Map<List<ProductDTO>>(products).ToList().OrderBy(x => x.Name).ToList();
            return productDto.AsResponse<IEnumerable<ProductDTO>>(200, "");
        }
